...
// Try "public/" first, fall back to root directory
...
// Check if public/ exists, otherwise serve from root
const staticRoot = existsSync(publicDir) ? "./public" : ".";
app.use("*", serveStatic({ root: staticRoot }));

// Fallback: serve index.html for all non-API routes (SPA support)
app.get("*", (c) => {
  const indexPath = existsSync(publicDir) ? join(publicDir, "index.html") : join(rootDir, "index.html");
  try {
    const html = readFileSync(indexPath, "utf-8");
    return c.html(html);
  } catch {
    return c.json({ error: "index.html not found. Please upload index.html to your repository." }, 500);
  }
});
